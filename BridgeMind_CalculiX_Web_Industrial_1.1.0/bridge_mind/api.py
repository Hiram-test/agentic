...
def create_app(database_path: str | Path | None = None) -> FastAPI:  # 创建可测试和可部署的 FastAPI 应用。
    ...
    service.seed_templates()  # 确保结构问卷、网格策略和经验摘要模板存在。
    app = FastAPI(title="BridgeMind CalculiX Web", version=PACKAGE_VERSION, description="BSDL Industrial、移动触控、CalculiX 2.23、IFC 4.3、版本审计和经验闭环。")  # 创建 API 应用。
