...
function renderProjectSummary() { // 显示当前 BSDL 文档的核心规模和状态。
    ...
    elements.projectSummary.innerHTML = `<strong>${escapeHtml(state.document.project?.name || state.projectId)}</strong><br>修订 V${state.document.revision?.number || "?"} · 节点 ${(state.document.nodes || []).length} · 构件 ${(state.document.components || []).length}<br>活跃区域 ${activeRegions.length}（${bySource}）<br>任务 ${(state.document.analysisTasks || []).map((task) => task.name).join("、") || "未定义"}`; // 显示项目摘要。
} // 结束项目摘要函数。
...
async function initialize() { // 初始化界面、事件和默认项目。
    ...
    try { // 分阶段检测公开端点和受保护项目数据。
        ...
        try { await Promise.all([loadTemplates(), loadProjects()]); } catch (error) { if (error.status === 401) { updateApiKeyStatus(true, "请输入生产环境 API 密钥后加载项目。" ); openMobilePanel("left"); } else { throw error; } } // 在需要密钥时保留网站可操作状态并打开访问面板。
